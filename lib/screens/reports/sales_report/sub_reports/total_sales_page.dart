...
import 'package:zed_nano/services/pdfs/SalesReportService.dart';
...
class _TotalSalesPageState extends State<TotalSalesPage> {
  ...
  Future<void> _showGenerateReport() async {
    ...
    final startDate = dateRange.values.first.removeTimezoneOffset;
    final endDate = dateRange.values.last.removeTimezoneOffset;
    await SalesReportService.generateSalesReportPdf(
      context,
      ...
      if (value != null) {
        await PdfPage(
          pdfBytes: value,
          title: 'Total Sales Report - ${startDate.toDateOnly} to ${endDate.toDateOnly}',
          fileName: 'Total Sales Report - ${startDate.toDateOnly} to ${endDate.toDateOnly}.pdf',
        ).launch(context);
      }
    });
  }
