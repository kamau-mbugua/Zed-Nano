...
class PendingVoidedTransactionsPage extends StatefulWidget {
  const PendingVoidedTransactionsPage({super.key});

  @override
  State<PendingVoidedTransactionsPage> createState() => _PendingVoidedTransactionsPageState();
}

class _PendingVoidedTransactionsPageState extends State<PendingVoidedTransactionsPage> {
  ...
  Future<List<VoidPendingTransaction>> getVoidedTransactionsPending({required int page, required int limit}) async {
    final response = await getBusinessProvider(context).getVoidedTransactionsPending(
      page: page,
      limit: limit,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: const AuthAppBar(title: 'Pending Voided Transactions'),
              ...
              child: PagedListView<int, VoidPendingTransaction>(
                ...
                builderDelegate: PagedChildBuilderDelegate<VoidPendingTransaction>(
                  itemBuilder: (context, item, index) {
                    return addVoidPendingTransactionsApprovalItemBuilder(
                      item,
                      onApprove:(){
                        _handleDecline(
                          item.transactionID,
                      ...
                      VoidTransactionDetailPage(transactionId: item.transactionID).launch(context).then((value) {
                        _paginationController.refresh();
                      });                    });
                  },
